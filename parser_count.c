...
int		re_parser_subexpre(t_token *token, char *pat, int size_match)
{
	token->type = TKN_TREE; // Do recreate full tree from regex shouldn't be here
}

void	re_parser_count_range(t_token *token, char *pat, int size_match)
{
	int		index;
	char	selecteur;
...
int		re_parser_count(t_token *token, char *pat, int size_match)
{
	token->type = TKN_OP;
	if (*pat == '*')
		token->data.op.type = TKN_OP_STAR;
	else if (*pat == '?')
		token->data.op.type = TKN_OP_QUERY;
	else if (*pat == '+')
		token->data.op.type = TKN_OP_PLUS;
	else
	{
		printf("Range count\n");
		token->data.op.type = TKN_OP_RANGE;
		re_parser_count_range(token, pat + 1, size_match);
		printf("Min %d Max %d\n", token->data.op.min, token->data.op.max);
	}
}